...
def query_freight_value_weight_relationship(database: Engine) -> QueryResult:
    ...
    df1 = pd.merge(orders,items, on = 'order_id', how="inner",  suffixes=("_item", "_product"))
    data = pd.merge(df1, products, on = 'product_id', how="inner")

    mask = data['order_status'] == 'delivered'
    delivered = data[mask]


    aggregations =   aggregations = delivered.groupby("order_id" ).agg(order_id=("order_id", "first"),
...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    mask = orders["year"] == 2017 
    filtered_dates = orders[mask]

  
    filtered_dates['date'] = filtered_dates["order_purchase_timestamp"].dt.date
    order_purchase_ammount_per_date = filtered_dates.groupby('date', as_index= False).count()


    holidays['date'] = pd.to_datetime(holidays["date"])
    df = pd.DataFrame()
    df[['order_count', 'date']] = order_purchase_ammount_per_date[['order_id', 'date']]
    df['holiday'] = (pd.to_datetime(df['date'])).isin(holidays['date'])
    result_df = df
