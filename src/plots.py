...
def plot_freight_value_weight_relationship(df: DataFrame):
    ...
    """
    sns.scatterplot(x =df['product_weight_g'], y = df['freight_value']).set(
        title="Relationship Between freight and weighta")
...
def plot_order_amount_per_day_with_holidays(df: DataFrame):
    ...
    ax = plt.plot(df['date'], df['order_count'], color = 'green') 
    for d in df.loc[df['holiday'] ==True, 'date']:
     plt.axvline(d, color = 'blue', linestyle = ':')
    plt.show()
